...
setSampleParameters{ // call to super from TimSample 3
...
hardPlay{ arg tcDuration;
		...
		t.sched( (tcDuration  /*- 0.03  */),   {
			this.synth.free; // frees the stored instacne
...
softPlay{ arg tcDuration;
			...
			t.sched( tcDuration + (this.duration/100),
				{this.setSampleLoopTo(0); t.clear;
				 this.name.debug(" SOFTSTOP" + "node ID" + synth.asNodeID.asString);
		     }
		)
		}
	 }
...
duration{
		...
		{aDuration = this.buffer.duration*this.tempo}.try
...
neededRepeatsFor{
	...
	//choose {}	//for common protocol with choosers for artisinal seqiecnes
